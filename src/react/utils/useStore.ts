 ...
 *
 * Instead of using a snapshot, and proxy-comparing against that, we access proxy
 ...
 * Current pros/cons:
 *
 * - Pro: removes the complexity/ergonomics of separate snap/store
 ...
 * - Con: Doesn't use snapshots, and so is stuck with `useSES` Level 2
 *     semantics--enforced consistency, but potentially long renders.
 *
 * @example
...
function getStore<T extends object>(obj: T): T | undefined {
  return (obj as { [GET_STORE_SYMBOL]?: typeof obj })[GET_STORE_SYMBOL]
}
